"""
This script loads the preprocessed TEM dataset depending on the value of i:
1. Loads an trained model and provides inference
2. trains a new model and saves it
"""
import cv2
import numpy as np
...
INF_DIR = 'data_tem/AB/train/'
inf_ds = ImageFolder(INF_DIR, transform=transforms.Compose([
        transforms.Resize([762,1254]),
        transforms.ToTensor(),
        Val_Normalize()]))
inf_dl = DataLoader(inf_ds)
...
if i==1:
    ...
    #         print('fake_gen:', fake_gen)
        ...
        checkpoint_dir = "sem_gan/new_checkpnt_150ep/" 
        model_dir = "sem_gan/new_checkpnt_150ep"
        is_best = False
        if epoch== 1:
            ...
            save_image(train_inp, 'sem_gan/img_new_150ep/train_inp.png', normalize=True)
            checkpoint_g = {
                'epoch': epoch + 1,
        ...
        for (inputs, targets), _ in val_dl:  
            ...
            save_image(generated_output.data[:10], 'sem_gan/img_new_150ep/val_sample_%d'%epoch + '.png', nrow=5, normalize=True)
        ...
        for (real, condition), _ in test_dl:
                ...
                # fake2 = fake11.view(fake11.shape[1], fake11.shape[2], fake11.shape[3])
                save_image(fake1, 'sem_gan/img_new_150ep/SampleFake_%d'%epoch  + '.png', normalize=True)
                # save_image(fake2, 'tem_gan2/samples1500/SampleFake_%d'%epoch  + '.png', normalize=True)

                if c_check==1:
                    save_image(cond1, 'sem_gan/img_new_150ep/SampleCon_%d'%epoch  + '.png', normalize=True)
                    save_image(real1, 'sem_gan/img_new_150ep/SampleReal_%d'%epoch  + '.png', normalize=True)
                c_check = c_check+1 
...
if i==0:
    ...
    optim_G = G_optimizer 
    optim_D = D_optimizer 
    ckp_path_dis = "sem_gan/new_checkpnt_150ep/checkpoint_dis.pt"
    ckp_path_gen = "sem_gan/new_checkpnt_150ep/checkpoint_gen.pt"
    ...
    count = 1
    epoch = start_epoch1
    for (real, condition), _ in inf_dl:
        fake = generator(real)
        cond = real
        rel = condition
        # p1 = nn.MaxUnpool2d(3, stride=3)
        ...
        fake1 = fake.view(fake.shape[1], fake.shape[2], fake.shape[3])
        # fake2 = fake11.view(fake11.shape[1], fake11.shape[2], fake11.shape[3])
        save_image(fake1, 'sem_gan/test_150ep/test2/%d'%count  + '.png', normalize=True)
        save_image(cond1, 'sem_gan/test_150ep/test2/SampleCon_%d'%count  + '.png', normalize=True)
        save_image(real1, 'sem_gan/test_150ep/test2/SampleReal_%d'%count  + '.png', normalize=True)
        # save_image(fake2, 'tem_gan2/samples1500/SampleFake_%d'%epoch  + '.png', normalize=True)
        count  = count + 1
        if c_check==1:
            save_image(cond1, 'sem_gan/test_150ep/SampleCon_%d'%epoch  + '.png', normalize=True)
            save_image(real1, 'sem_gan/test_150ep/SampleReal_%d'%epoch  + '.png', normalize=True)
        c_check = c_check+1
